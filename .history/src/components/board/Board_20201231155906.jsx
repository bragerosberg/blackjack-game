...
const Board = () => {
  ...
  const [dealerCardCounter, updateDealerCardCounter] = useState(0);
      ...
      <section>
        <aside className="blackjack__dealer__wrapper">
          {allDealerCards.map(dealerCard => (
            updateDealerCardCounter(dealerCardCounter + 1)
            <Dealer number={dealerCardCounter} key={dealerCard.id} card={dealerCard}/>
          ))}
        </aside>
